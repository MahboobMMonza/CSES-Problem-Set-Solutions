...
int main() {
    ...
    int n, priceCap;
    cin >> n >> priceCap;
    vector<int> prices(n), pages(n), bestPages(priceCap + 1, 0);
    for (int i = 0; i < n; ++i) {
        cin >> prices[i];
    }
    for (int i = 0; i < n; ++i) {
    ...
    // Classic 0-1 knapsack -- iterate in reverse because the row is technically optimal
    fora(i, 0, n, 1) {
        forde(j, priceCap, prices[i], 1) {
            bestPages[j] = max(bestPages[j], bestPages[j - prices[i]] + pages[i]);
        }
    }
    cout << bestPages[priceCap] << edl;
    return 0;
}