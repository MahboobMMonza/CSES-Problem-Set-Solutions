...
int main() {
    fio;
    int n, moneySums = 0, numSums = 0;
    ostringstream ss;
    cin >> n;
    vector<int> coins(n);
    bitset<(int) 10e5 + 1> sums(false);
    for (int i = 0; i < n; ++i) {
        cin >> coins[i];
        moneySums += coins[i];
    }
    sums[0] = true;
    // Same approach to subset sum problem with space optimization by looping in reverse up to coins[i]
    fora(i, 0, n, 1) {
        forde(j, moneySums, coins[i], 1) {
            sums[j] = sums[j - coins[i]] || sums[j];
        }
    }
    forae(i, 1, moneySums, 1) {
        if (sums[i]) {
            numSums++;
            ss << i;
            if (i != moneySums) {
