...
int main() {
    ...
    if ((sum - 1) & 1) {
        sum >>= 1;
        ways[0] = 1;
        // Count number of ways not using the last number in the range. Each subset pair either has the last number, or
        // doesn't. Count the ones that don't, and you've counted all the pairs of subsets.
        fora(i, 1, n, 1) {
            forde(j, sum, i, 1) {
                ways[j] += ways[j - i];
                ways[j] %= MOD;
