...
vector<vector<int>> getPaths(const size_t n, const size_t num_routes, vector<int> &trav_heads, vector<edge> &edges) {
    vector<vector<int>> paths(num_routes, vector<int>());
    foraz(i, 0, num_routes) {
        vector<int> path;
        path.pb(0);
        while (true) {
            const auto cur = path.bk;
            if (cur == static_cast<int>(n) - 1) {
                break;
            }
            while (trav_heads[cur] != -1) {
                auto &e = edges[trav_heads[cur]];
                const auto idx = trav_heads[cur];
                trav_heads[cur] = e.nxt;
                if (e.getCapacity() != 0 || idx & 1) continue;
                e.flow -= 1;
                path.pb(e.to);
                break;
            }
...
int main() {
    /**
     * Use max flow to calculate maximum number of edge routes in the network, using the non-recursive implementation
     * of Dinic's and setting each edge weight to 1. Then once the flow is found, conduct DFS' from source to sink,
     * resetting edge weights to 0 as you go until no more filled edges remain. Make sure to only go through ORIGINAL
     * edges, which have even edge indices. The DFS will always move forward through the flow graph, so just add the
     * current path to a list, and use destructive heads to prevent duplicate checks of edges.
     */
    fio;
    size_t n, m;
    cin >> n >> m;
    ...
    const auto num_paths = getFlow(heads, trav_heads, level, edges, searching);
    copy(heads.begin(), heads.end(), trav_heads.begin());
    const auto paths = getPaths(n, num_paths, trav_heads, edges);
    cout << num_paths << edl;
