...
int main() {
     ...
     * until the starting node is node 1, then update the cost of each of the neighbours to add the cost of the current
     * node, as well as accounting for modulo. If the neighbour's cost is 0, then set it to be the cost of the current
     * node.
     */
    fio;
    int n, m, a, b;
    ...
    forde(i, n - 1, 0) {
        ...
        for (int idx = heads[cur]; idx != -1; idx = edges[idx].nxt) {
            ...
            if (dists[nbr] == 0) {
                ...
                dists[nbr] = (dists[cur] + dists[nbr]) % MOD;
            }
        }
    }
    cout << dists.back() << edl;
