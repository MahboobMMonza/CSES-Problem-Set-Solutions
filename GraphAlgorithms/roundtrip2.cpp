...
public:
    virtual bool
    execute(int a, vector<int> &heads, vector<edge> &edges, vector<ll> &marks, vector<int> &pred,
            st<pair<int, ICommand &>> &stk, vector<int> &order) = 0;
...
public:
    bool execute(int a, vector<int> &heads, vector<edge> &edges, vector<ll> &marks, vector<int> &pred,
                 st<pair<int, ICommand &>> &stk, vector<int> &order) override {
...
bool dfs(int a, vector<int> &heads, vector<edge> &edges, vector<ll> &marks, vector<int> &pred,
         st<pair<int, ICommand &>> &stk, vector<int> &order, ICommand &complete, ICommand &visit) {
    ...
    while (!stk.empty()) {
        ...
        int cur = stk.tp.fs;
        stk.pp;
        bool res = command.execute(cur, heads, edges, marks, pred, stk, order);
        if (res) {
            return true;
        }
    }
...
inline bool
findCycle(vector<int> &heads, vector<edge> &edges, vector<ll> &marks, vector<int> &pred,
          st<pair<int, ICommand &>> &stk, vector<int> &order) {
    ...
    fora(i, 0, heads.size()) {
        ...
        if (dfs(i, heads, edges, marks, pred, stk, order, complete, visit)) {
            return true;
        }
    }
    return false;
...
int main() {
    ...
    int n, m, a, b;
    cin >> n >> m;
    vector<int> heads(n, -1), pred(n, -1), order;
    ...
    if (!findCycle(heads, edges, marks, pred, stk, order)) {
        cout << "IMPOSSIBLE" << edl;
        return 0;
    }
    cout << order.size() << edl;
