...
public:
    ...
    void update(size_t i, T value) {
        ...
        rangeData[i] = value;
        for (i >>= 1; i > 0; i >>= 1) {
            rangeData[i] = op(rangeData[i << 1], rangeData[(i << 1) ^ 1]);
        }
    }
...
int main() {
    ...
    size_t idx;
    cin >> n;
    vector<int> data(n), status(n, 1);
    int msb = 31 - __builtin_clz(n);
    if (n ^ (1<<msb)) {
        status.resize((1 << (msb + 1)));
    }
    fora(i, 0, n) {
        cin >> data[i];
    }
    SegmentTree<int> seg(status, add_int, greater_equal<int>(), 0);
    fora(i, 0, n) {
        cin >> a;
        ans = seg.findFirst(a);
        idx = ans.fs - status.size();
        cout << data[idx];
        seg.update(idx, 0);
        cout << ((i <= n - 2) ? ' ' : edl);
    }
