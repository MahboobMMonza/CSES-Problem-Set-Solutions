...
int main() {
    /**
     * Maybe just a multiset can be used?
     */
    fio;
     int n, q;
     char c;
     ...
     indexed_set<pi> salSet;
    fora(i, 0, n) {
        cin >> salaries[i];
        salSet.insert({salaries[i], i});
    }
    while (q--) {
        cin >> c >> a >> b;
        if (c == '!') {
            --a;
            salSet.erase({salaries[a], a});
            salaries[a] = b;
            salSet.insert({salaries[a], a});
        } else {
            auto l = salSet.order_of_key({a, 0});
            auto u = salSet.order_of_key({b, n});
            size_t ans = u - l;
            cout << ans << edl;
        }
    }
