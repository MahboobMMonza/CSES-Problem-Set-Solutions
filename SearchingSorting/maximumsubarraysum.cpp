...
int main() {
    /**
     * At each number, the maximum subarray that includes that number is either a continuation of the previous max
     * subarray, or a completely new subarray. This is because subarrays are contiguous, meaning no values can be
     * skipped. If it is better to skip that value, then the current subarray must end, and a new one will be
     * considered. Keep track of the max value that is found in each case, and that will be the answer.
     */
    fio;
    // const int MX = 2e5+5;
    int n;
    ll s, ans = numeric_limits<ll>::min(), w;
    cin >> n;
    cin >> w;
    ans = s = w;
    fora(i,1,n,1) {
