...
int main() {
    ...
    ll day = 0, reward = 0;
    cin >> n;
    vector<pil> tasks(n); // {duration, deadline}
    for (int i = 0; i < n; ++i) {
        cin >> tasks[i].fs >> tasks[i].sc;
    }
    // Always do the shortest tasks first. There is also less penalty for earlier deadlines if they are done first.
    // Natural sorting of the pair automatically sorts this way.
    sort(tasks.begin(), tasks.end());
    for (int i = 0; i < n; ++i) {
        day += tasks[i].fs;
        reward += tasks[i].sc - day;
    }
    cout << reward << edl;
    return 0;
}