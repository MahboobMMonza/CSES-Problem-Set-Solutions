...
int main() {
    /**
     * Use an unordered map to track the frequency of distinct values. If freq becomes 0, remove the element. When
     * k = 1, always 1 distinct element. Insymotion-prefix) order for unordered map to work a custom hash function
     * (splitmix) needs to be used to avoid being hacked.
     */
    fio;
    unordered_map<int, int, custom_hash> dvs;
    int n, k, cnt = 0;
    cin >> n >> k;
    if (k == 1) {
        forae(i, 0, n - k) {
